...
async function testScooterResult() {
    ...
    for (let i = 1; true; i++) {
        ...
        const nextMetroField = await page.$x('//*[@id="root"]/div/div[2]/div[2]/div[4]/div/div[2]/ul/li['+i+']/button'); //тут хитрость. Чтобы корректно выбрать станцию, мы обращаемся к ней через xpath локатор. в каждом последующем локаторе атрибут "data-index" увеличивается на 1, ровно как переменная-счетчик!
        ...
        // иногда номер заказа не успевает отобразиться. На этот случай существует костыль проверяющий, что в переменной не null. Если номер не вывелся, мы прогоняем итерацию повторно
        if (orderNumber === null) {
            i -= 1;
