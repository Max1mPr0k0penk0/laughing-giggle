...
async function testScooterResult() {
    ...
    for (let i = 1; true; i++) {
        ...
        // тут следует сложный момент, который я подглядел на stackowerflow. Как я понимаю, мы получаем все элементы по селектору, перебераем в цикле и достаем значение атрибута value
        let nameOfStation = []; 
